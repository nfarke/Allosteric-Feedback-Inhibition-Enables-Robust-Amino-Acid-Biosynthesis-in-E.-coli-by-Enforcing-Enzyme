...
[p,~] = Sample(1,0); %parameter sampling
...
parfor i1 = 1:ParSize
    ...
    while value == 0
        ...
        [t,y]  =  ode23s(@(t,c) odemodel(t,c,p,par),tspan,y0,opts);
        ...
        if SS<1E-08 && ew<-1E-5 && t(end) == tspan(end)  && min(min(y))>0
            PAR(:,i1) = par;
...
function dcdt  =  odemodel(~,c,p,par)
